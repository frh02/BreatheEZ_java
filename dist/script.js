...
function onResults(results) {
    ...
    if (results.poseLandmarks) {
        ...
        if (results.leftHandLandmarks) {
            canvasCtx.strokeStyle = 'white';
            connect(canvasCtx, [[
                    results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_SHOULDER],
                    results.rightHandLandmarks[0]
                ]]);
        }
        if (results.leftHandLandmarks) {
            canvasCtx.strokeStyle = 'white';
            connect(canvasCtx, [[
                    results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_SHOULDER],
                    results.leftHandLandmarks[0]
                ]]);
        }
    ...
    drawingUtils.drawConnectors(canvasCtx, results.faceLandmarks.landmark[13].x, results.faceLandmarks.landmark[13].y,
        {color: '#C0C0C070', lineWidth: 1});
    drawingUtils.drawConnectors(canvasCtx, results.faceLandmarks.landmark[14].x, results.faceLandmarks.landmark[14].y,
        {color: '#C0C0C070', lineWidth: 1});
    canvasCtx.restore();
}
